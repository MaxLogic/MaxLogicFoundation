...
begin
  ...
  if ACaseSensitive then
    FComparer := TEqualityComparer<string>.Ordinal
  else
    FComparer := TEqualityComparer<string>.OrdinalIgnoreCase;
end;
...
{$ELSE}
  ...
  begin
    ...
    begin
      ErrMsg := Format('Command exited with code %d', [WEXITSTATUS(Status)]);
      Result := False;
    end;
  end
  else if WIFSIGNALED(Status) then
  begin
    ErrMsg := Format('Command terminated by signal %d', [WTERMSIG(Status)]);
    Result := False;
  end
  else
  begin
    ErrMsg := 'Command did not exit normally';
    Result := False;
  end;
{$ENDIF}
  Output := Output.Trim;
...
{$ENDIF}
  ...
  procedure AddRoot(const Kind: TSearchRootKind; const Path: string);
  var
...
begin
  ...
  Seen := TDictionary<string, Byte>.Create(TKeyComparer.Create(not IsWindowsPlatform));
  try
    BasePath := BaseDir;
    if BasePath = '' then
      BasePath := TDirectory.GetCurrentDirectory;
    BasePath := TPath.GetFullPath(BasePath);
    AddRoot(TSearchRootKind.srCWD, BasePath);
    ...
    begin
      ...
      begin
        ...
        if (ParentPath = '') or SameFileName(ParentPath, Current) then
          Break;
        AddRoot(TSearchRootKind.srParents, ParentPath);
        Current := ParentPath;
      end;
    end;
...
{$ENDIF}
    ...
    begin
      ...
      if HomeDir <> '' then
      begin
        AddRoot(TSearchRootKind.srHome, HomeDir);
        AddRoot(TSearchRootKind.srHome, TPath.Combine(TPath.Combine(HomeDir, '.config'), 'maxlogic'));
      end;
    end;
...
{$IFDEF MSWINDOWS}
    ...
    begin
      ...
      if ParentPath <> '' then
        AddRoot(TSearchRootKind.srWinProfile, TPath.Combine(ParentPath, 'MaxLogic'));
    end;
{$ENDIF}
...
begin
  ...
  if Target = '' then
    Exit;
  if not TPath.IsPathRooted(Target) then
  begin
    if BaseFile <> '' then
...
begin
  ...
  if stat(PAnsiChar(AnsiString(FilePath)), StatBuf) <> 0 then
  begin
    AddError(FilePath, 0, 0, TDotEnvErrorKind.DekIO, SysErrorMessage(errno), True);
    Exit(False);
  end;
